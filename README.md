...
##### 针对不同的输入参数，动态生成奖牌者的分布和分析（Chart H L）

<p>Chart H展示了奖牌得主的体重、身高和年龄的分布情况。图表使用面积图和散点图展示了不同变量（体重、身高、年龄）的奖牌得主数量，并通过facet_wrap按照变量分别展示。</p>
<p>数据处理与准备：</p>
<p>选择数据： 从原始数据中选择性别、运动、体重、身高、年龄和奖牌总数等相关列的数据。</p>
<p>筛选数据： 根据用户在侧边栏选择的性别和运动进行筛选，确保只包括符合条件的奖牌得主数据。</p>
<p>数据重塑： 使用pivot_longer将体重、身高和年龄列进行整理，使得这些变量都在一个列中。</p>
<p>聚合数据： 使用aggregate函数对每个变量的奖牌总数进行求和，以便后续绘图。</p>
<p>图表绘制：</p>
<p>绘制面积图： 针对每个变量，使用面积图展示了奖牌得主数量的分布情况。不同颜色的区域表示不同的变量。</p>
<p>绘制散点图： 在面积图上使用散点图标识每个具体的奖牌得主数量，以更清晰地显示数据。</p>
<p>分面展示： 使用facet_wrap函数，按照变量分别展示不同的子图，使得每个变量的分布情况都能独立展示。</p>
<p>调整图表样式： 调整了图表的主题、颜色、标签等元素，以提高可读性和美观性</p>


<p>Chart L通过词云图展示奥运会运动员的分布情况，以及各个国家运动员的奖牌情况。词云图是一种直观的数据可视化方式，通过词汇的大小和颜色来反映词频的大小，帮助观众更容易理解数据的分布和特点。</p>
<p>词云图的制作需要指定词汇（在这里是国家）、词频等参数。这里以国家为单位，词频为运动员的总奖牌数。通过调整参数，可以设置词云图的外观，包括颜色、大小、最大词数等</p>
